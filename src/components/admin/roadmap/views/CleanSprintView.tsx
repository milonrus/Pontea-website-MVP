...
const buildSubjectGroups = (
  ...
  for (const item of sprint.source.items) {
    ...
    const name = item.submodule_name?.trim() || 'Untitled module';
...
const CleanSprintView: React.FC<VisualRoadmapViewProps> = ({ roadmap }) => {
  const allSubjects = buildOrderedSubjects(roadmap);
              ...
              >
                ...
                <div className="px-4 pb-4 space-y-2.5">
                  ...
                  {activeSubjects.length === 0 && (
                    <p className="text-[13px] text-slate-400 italic">No modules</p>
                  )}
                </div>
              </div>
            );
...
export default CleanSprintView;
