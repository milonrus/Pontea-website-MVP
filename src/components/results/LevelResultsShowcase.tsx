...
const DOMAIN_LABEL_RU: Record<string, string> = {
  reading_en: 'Чтение',
  logic: 'Логика',
...
const getLabel = (d: DomainResult) => DOMAIN_LABEL_RU[d.domain] ?? d.domainLabel;

const pluralWeeks = (n: number): string => {
  const mod10 = n % 10;
  const mod100 = n % 100;
...
const LevelResultsShowcase: React.FC<LevelResultsShowcaseProps> = ({ domainResults, weeksUntilExam, focusRecommendation }) => {
      ...
      {weeksUntilExam != null && (
        <div className="flex-shrink-0 w-64 rounded-2xl bg-white shadow-sm border border-slate-200/60 p-6 flex flex-col items-center justify-center text-center">
          ...
          <p className={`text-xs mt-4 leading-relaxed max-w-[200px] ${focusRecommendation ? 'text-slate-600' : 'text-slate-400'}`}>
            {focusRecommendation ?? 'Мы составили детальный план под твой уровень знаний. Начни готовиться уже сейчас.'}
          </p>
